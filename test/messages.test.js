...
describe('Anthropic messages request translation', () => {
  ...
  it('drops unsupported Anthropic server-side tool types and converts web_search before forwarding', async () => {
    let capturedBody = null;
    await handleMessages({
      model: 'claude-sonnet-4.6',
      tools: [
    ...
    // advisor/code_execution are stripped; web_search_20250305 is now
    // converted to a function tool that Cascade can bridge.
    assert.equal(capturedBody.tools?.length, 2);
    const names = capturedBody.tools.map(t => t.function.name);
    assert.deepEqual(names, ['web_search', 'Read']);
