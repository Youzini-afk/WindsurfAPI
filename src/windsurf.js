...
import { getSystemPrompts, isExperimentalEnabled } from './runtime-config.js';
...
function buildCascadeConfig(modelEnum, modelUid, { toolPreamble, forceDefault, nativeMode, nativeAllowlist } = {}) {
  ...
  if (toolPreamble) {
    ...
    const reinforcement = isExperimentalEnabled('toolReinforcementPrompt')
    ...
    // field 12 (additional_instructions_section).
    if (isExperimentalEnabled('communicationSectionPrompt')) {
    ...
    if (isExperimentalEnabled('noToolGuardPrompt')) {
